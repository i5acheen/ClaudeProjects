...
import type { ControlCategory, ControlStatusValue, EvidenceType } from "@/lib/types";

// Shared by prisma/seed.ts (local `prisma migrate dev`) and lib/runtime-init.ts
// (lazy seeding into /tmp on a read-only-filesystem host like Vercel), so the
// demo data only lives in one place.

export const TRAINING_LOG_CSV = `employee_name,employee_email,course,completed_at,score
Priya Nair,priya@aurorarobotics.example,Security Awareness 2026,2026-02-03,100
Marcus Webb,marcus@aurorarobotics.example,Security Awareness 2026,2026-02-04,95
Dana Okafor,dana@aurorarobotics.example,Security Awareness 2026,2026-02-04,100
Leah Simmons,leah@aurorarobotics.example,Security Awareness 2026,2026-02-10,90
Owen Park,owen@aurorarobotics.example,Security Awareness 2026,2026-02-10,100
`;
...
const CONTROLS: {
  ...
  },
  {
    ...
    description: "All employees re-acknowledge the Acceptable Use Policy at least once every 12 months.",
    category: "POLICY",
  },
  {
    code: "PPL-09",
...
export async function runSeed(prisma: PrismaClient, evidenceRoot: string): Promise<void> {
  const passwordHash = await hashPassword("ComplyTrail123!");
  ...
  for (const [code, plan] of Object.entries(plans)) {
    ...
    if (plan.evidence) {
      ...
      if (!existing) {
        if (plan.evidence.type === "FILE") {
          const filePath = path.join(evidenceRoot, plan.evidence.content);
          fs.mkdirSync(path.dirname(filePath), { recursive: true });
          if (!fs.existsSync(filePath)) fs.writeFileSync(filePath, TRAINING_LOG_CSV);
        }
